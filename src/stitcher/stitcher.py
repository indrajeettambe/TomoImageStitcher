...
class Stitcher:
    ...
    def get_layers_in_z(self, tolerance_mm):
        ...
        if type(self.global_coord_x_y_z) == type(None):
    ...
    def check_padding(self, layer_index=0, circular_mask=False, radius_mask=None):
        ...
        for j, file_path in enumerate(self.layers_paths[i]): # the iterator j is used to acces the padding values on each slice
            ...
            if circular_mask: extracted_slice = np.where(Utilities.circular_mask(extracted_slice, radius=radius_mask, center=(0,0)), extracted_slice, 0)
    ...
    def extract_intersection(self, start_slice = None, end_slice = None, layer=0, image=0, intersection=0, mask=False, mask_radius=None):
        ...
        """
        if type(self.layers_intersecting_images_bb_x_y) == type(None): sys.exit("Make sure to have run the get_intersections() method first")
        if type(mask_radius) == type(None): mask_radius = self.radius
        ...
        if mask:
            ...
            if start_slice == end_slice: intersection_bb = intersection_slice[y_0:y_1, x_0:x_1]
            else: intersection_bb = intersection_slice[:, y_0:y_1, x_0:x_1]
    ...
    def correlate_intersection(self, start_slice = None, end_slice = None, crop_x=(0,0), crop_y=(0,0), equal_crop_xy = None, reduce_slices_LC=None, layer=0, image=0, intersection=0, mask=False, mask_radius=None, verbose=False, downscale=0.5, downscale_stages=2, downscale_LC=False, spline_interp=False, apply_mean_filter_zyx=(0,0,0), apply_detrend_filter_yx=(0,0), apply_affine_warp=False, keep_rigid_only=True):
        ...
        if type(equal_crop_xy) != type(None):
        ...
        if (type(reduce_slices_LC) != type(None)) and (reduce_slices_LC != 0):
            # Extract the intersections
        ...
        x_0, y_0 = np.add(self.layers_intersecting_images_bb_x_y[layer][image][intersection_index][0], (self.padding_neg_x[layer][image] + crop_x[0], self.padding_neg_y[layer][image] + crop_y[0]))
            # z_0
        if type(reduce_slices_LC) != type(None):
    ...
    def compute_shift_in_layers(self, start_slice=None, end_slice=None, crop_x=(0,0), crop_y=(0,0), equal_crop_xy = None, reduce_slices_LC=None, mask=False, mask_radius=None, verbose=False, save_reg=False, save_path=None, downscale=0.5, downscale_stages=2, downscale_LC = False, spline_interp=False, apply_mean_filter_zyx=(0,0,0), apply_detrend_filter_yx=(0,0), apply_affine_warp=False, keep_rigid_only=True):
        ...
        if save_reg:
            if (type(save_path) == type(None)) and (type(self.saving_path) == type(None)):  # Better way to check for None
        ...
        for i, layer_indices in enumerate(self.layers_intersecting_images_indices):
            ...
            for j, reference_subvolumes in enumerate(layer_indices):
                ...
                trace_results = 0
                for l, job_status in enumerate(current_jobs_status):
                    moving_intersection_indice = reference_subvolumes[l] # get the row to be synchronize the current loop to the previous one
                    if job_status == 1:
                        ...
                        if save_reg:
                            ...
                            with h5py.File(registration_saving_path + f"/registration_{i}_{j}_{l}.h5", "w") as h5f: 
                            ...
                            with h5py.File(registration_saving_path + f"/registration_NCC_{i}_{j}_{l}.h5", "w") as h5f: h5_file = h5f.create_dataset("image", data=np.array(registered_slices))
    ...
    def get_displacement_pyramid(self, check=False, starting_coord = None):
        ...
        for starting_layer, i in enumerate(self.layers_intersecting_images_indices):
            if (starting_layer == 0) and (type(starting_coord) == type(None)):
                        ...
                        if expansion_layer > 1: # Starting from 1 as the fixed subvolume does not have neighbors nor displacement nor ncc.
                            ...
                            if current_subvolume not in unique_volumes_index:
                                ...
                                for i, neighbor in enumerate(previous_neighbors):
                                    ...
                                    transformation_center = np.array(cumulative_cent_pyramid_sub[expansion_layer - 2])[displacement_pos]
    ...
    def stitch_volumes_blend_equalize(self,
        ...
        if use_equalize: self.layer_equalize_slope_intercept = []
        ...
        if type(stitch_layer) == type(None): iterator = [i for i, layer_paths in enumerate(self.layers_paths)]
        else: iterator = (stitch_layer,)
        ...
        if len(iterator) > 1: collected_slices = []
        ...
        for _, stitch_layer_ in enumerate(iterator):
            ...
            for i, slice_index in enumerate(stitching_index_order[:-1]): # the iterator i is used to acces the padding values on each slice
                ...
                if already_passed_index[j_1] is None:
                    ...
                    if (self.layer_final_NCC[stitch_layer_][j_1] > self.exclude_NCC) or (exclude_NCC == False):
                        if type(self.affine_warp) == type(None) or  self.affine_warp == False:
                        ...
                        if distance_map_red.size == 0: distance_map_red = np.array((0))
                ...
                if show_progress_bar: progress_bar.update(1)  # Update the progress bar on every iteration
            
            if use_equalize: self.layer_equalize_slope_intercept.append(deepcopy(layer_equalize_slope_intercept_))
            
            if show_progress_bar: progress_bar.close()  # Close the progress bar after finishing all iterations
            
            if len(iterator) > 1:
                collected_slices.append(deepcopy(pad_x_y_j[:, crop_y[0]:pad_x_y_j.shape[1]-crop_y[1], crop_x[0]:pad_x_y_j.shape[2]-crop_x[1]]))
        if len(iterator) == 1: return pad_x_y_j[:, crop_y[0]:pad_x_y_j.shape[1]-crop_y[1], crop_x[0]:pad_x_y_j.shape[2]-crop_x[1]]
        else: return collected_slices
    ...
    def get_transformed_slices(self, layer=0, image=0, start_slice=None, end_slice=None, mask=False, mask_radius=None):
        ...
        """
        if ((start_slice == end_slice)) and (type(start_slice) != type(None)): sys.exit("For single slices use start_slice + 1 as end_slice!")
        ...
        if lower_boundary > self.img_depth: lower_boundary = self.img_depth
        if upper_boundary < 0: upper_boundary = 0
        if upper_boundary > self.img_depth: upper_boundary = self.img_depth
        ...
        if upper_boundary - 1 >= lower_boundary:
            ...
            if (img_slices.shape[0] != (end_slice - start_slice) + 4) and (reference_slice-2 >= 0): img_slices = np.pad(img_slices, ((0, (end_slice - start_slice) + 4 - img_slices.shape[0]), (0, 0), (0, 0)))
            if (img_slices.shape[0] != (end_slice - start_slice) + 4) and (reference_slice-2 < 0): img_slices = np.pad(img_slices, (((end_slice - start_slice) + 4 - img_slices.shape[0], 0), (0, 0), (0, 0)))
            # Transform the slice
            ...
            # Apply a mask if requested
            if mask: img_slices = np.where(Utilities.circular_mask(img_slices, 
                                                                   radius=mask_radius, 
    ...
    def get_transformed_slices_affine(self, layer=0, image=0, start_slice=None, end_slice=None, mask=False, mask_radius=None, chunk_size=None):
        ...
        if len(img_slices) != 1: img_slices = np.squeeze(img_slices)
        else: img_slices = img_slices[0]
        # Check for any mask
        if mask: img_slices = np.where(Utilities.circular_mask(img_slices, 
                                                               radius=mask_radius, 
                                                               center=(self.layer_final_displacements[layer][image][1], 
    ...
    def stitch_volumes_blend_equalize_parallel(self,
        ...
        """
        if type(stitch_layer) == type(None): sys.exit("You must specify the stiching layer!")
        if (type(self.layer_equalize_slope_intercept) == type(None)) or (type(use_existing_equalize) == type(None)): use_existing_equalize = False
    ...
    def stitch_layers(self, chunk_size_series = 200, chunk_size_parallel = 10, n_cores = 10, use_existing_equalize=None, path_save=None, check=False):
        ...
        """
        if (type(path_save) == type(None)) and (type(self.saving_path) == type(None)):
        ...
        for layer_inx, layer_paths in enumerate(self.layers_paths):
            ...
            with h5py.File(layers_saving_path + f"/Layer_{layer_inx}.h5", "w") as h5f:
                ...
                layer_info_dict = {"layer_coordinates" : np.squeeze(self.layers_coordinates[layer_inx]), 
                                   ...
                                   "layer_equalize_slope_intercept" : "None" if type(self.layer_equalize_slope_intercept) == type(None) else self.layer_equalize_slope_intercept[layer_inx]}
                ...
                for key in self.params_final.keys():
                    if type(self.params_final) == type(None): h5_file.create_dataset(key, data="None")
                    else: h5_file.create_dataset(key, data=self.params_final[key])
...
class Utilities:
    ...
    def circular_mask(img, radius=None, center=(0,0)):
        ...
        if radius == None: circular_mask = (img_radius <= im_dimension / 2)
        else: circular_mask = (img_radius <= radius)
    ...
    def normalize_distance_map_radially(distance_map, center=None):
        ...
        if distance_map.dtype != np.float32: distance_map = distance_map.astype(np.float32)

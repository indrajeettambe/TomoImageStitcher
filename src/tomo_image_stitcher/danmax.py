"""DanMAX beamline utilities.

This is the ``DanMAX.py`` file shipped with the Jupyter environment at the
DanMAX beamline (MAX IV Laboratory, Sweden). The module is optional and only
imported by the example notebooks. It exposes helpers for:

* discovering scans from a proposal/visit (``findScan``, ``findAllScans``)
* reading azimuthally-integrated diffraction data (``getAzintData``)
* reading scan metadata (``getMetaData``, ``getMetaDic``)
* interactive masking (``InteractiveMask``) and figure styling
  (``darkMode``/``lightMode``)

A handful of optional helpers rely on the beamline's ``lib.*`` packages
(``lib.mapping``, ``lib.texture``, ...). They are imported defensively and
...
class InteractiveMask():
    ...
    def updateRoi(self, roi, color):
        ...
        self.im.set_clim(vmin=self.image.min(), vmax=self.image.max())
        self.updateHist()
        plt.draw()
    ...
    def calcHistogram(self):
        ...
        hist, bins = np.histogram(self.image.flatten(), bins=512, density=True)
        cen = (bins[:-1] + bins[1:]) / 2
        return cen, hist

...
class affine_transform_large_data:
    ...
    def transform_chunk(self, chunk_index=0, use_mask=False):
        ...
        if isinstance(self.chunk_positions, type(None)):
            sys.exit("Please run the get_chunks_position() first to define the chunks!")
    ...
    def _check_operators(self):
        if isinstance(self.affine_transform_operator_x_y_z_1, type(None)):
            self.set_affine_transform_operator()
        if isinstance(self.affine_transform_center_x_y_z, type(None)):
            self.set_affine_transform_center()
        if isinstance(self.affine_transform_scaling, type(None)):
            self.scale_affine_transform()
        if isinstance(self.spline_order, type(None)):
            self.set_spline_order()
    ...
    def _check_bounds(self, z_min, z_max):
        ...
        if lower_bound > self.depth:
            lower_bound = self.depth
        if higher_bound > self.depth:
            higher_bound = self.depth
        # check if the lower or the higher bounds go beyond the image in -z direction
        if lower_bound < 0:
            lower_bound = 0
        if higher_bound < 0:
